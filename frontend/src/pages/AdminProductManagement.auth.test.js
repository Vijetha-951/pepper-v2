...
describe('AdminProductManagement - Authentication Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Default mock for productService to prevent additional API calls
  ...
  describe('Component Integration Tests', () => {
    test('Component loads products after successful authentication', async () => {
      // Setup: Valid authentication
      authService.getCurrentUser.mockReturnValue(mockValidAdmin);
      authService.getFirebaseUser.mockReturnValue(mockFirebaseUser);

      const mockProducts = [
        { _id: '1', name: 'Test Product', type: 'Bush', price: 100, stock: 50 }
      ];
      ...
      // Wait for authentication and product loading
      await waitFor(() => {
        expect(screen.getByText('Product Management')).toBeInTheDocument();
      });
    ...
    test('Component does not load products when authentication fails', async () => {
      ...
      // Verify products were not fetched due to auth failure
