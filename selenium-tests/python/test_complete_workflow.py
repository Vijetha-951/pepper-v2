"""
Comprehensive E-Commerce Workflow Tests
...
class TestCompleteWorkflow(BaseTest):
    """Test complete user journey: login, add to cart, checkout, and review"""
    
    def login_as_test_user(self):
        """Helper method to login with test user credentials"""
        ...
        print(f"[INFO] Using email: {Config.TEST_USER['email']}")
        ...
        try:
            ...
            try:
                ...
                if "/login" in current_url:
                    print("[WARN] Still on login page - user may not be registered yet")
                    print("[INFO] Note: Please register user account or update credentials in config.py")
                else:
                    print("[PASS] Login verified\n")
            except Exception as login_error:
                print(f"[WARN] Login failed: {login_error}")
                print("[INFO] Continuing with other tests if possible...")
            ...
            print("="*70)
            print("[PASS] COMPLETE JOURNEY TEST PASSED")
            print("="*70 + "\n")
            
        except Exception as e:
            print(f"\n[FAIL] COMPLETE JOURNEY TEST FAILED: {str(e)}\n")
            self.take_screenshot("complete_journey_failure")
            pytest.fail(f"Complete journey test failed: {str(e)}")
    ...
    def test_checkout_flow(self):
        """
        Test Case: Login, Add to Cart, and Checkout
        """
        try:
            print("\n" + "="*70)
            print("[CHECKOUT TEST]")
    ...
    def test_review_flow(self):
        """
        Test Case: Login and Leave Product Review
        """
        try:
            print("\n" + "="*70)
            print("[REVIEW TEST]")
